<?php
	class resizeImage {
		...
		function resizeImage($width,$height,$file,$resizeType="ratio",$quality=100){ // Constructer
			...
			$this->folderPath = str_replace($this->fileName,"",$this->imgFile);
			$this->thumbPath = $this->folderPath.'tmp/'.$width."_".$height."_".$this->resizeType."_".$this->fileName;
			
			if(true || !file_exists($this->thumbPath) || !is_file($this->thumbPath)){
		...
		function __resize($org, $_path) {		
			$resizeFunction = $this->resizeType;	
			//$tmpPath = $this->folderPath."tmp/";
			
			if(file_exists($org) && is_file($org)) {
				...
				$tmpImg = $this->fCallback["start"]($org);		
				...
				if($this->imgType == 1 ){
					$this->fCallback["end"]($newimg, $_path);
				} else {
					$this->fCallback["end"]($newimg, $_path,$this->quality);
				}
				@imagedestroy($newimg);
